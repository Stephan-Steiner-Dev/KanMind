...
Create a `.env` file in the `backend` root directory. You can use the provided `.env.example` file as a template.
...
The `.env` file contains sensitive configuration and is excluded from version control. Never commit your actual secret key to the repository.
